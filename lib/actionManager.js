...
async function playMediaOnClient(client,options) {
	...
	*/
	let targetClient = await searchForClient(client,options); // Return a valid, reasonable, targetable device client
	...
	*/
	let media = await searchForMedia(client,options); // Return valid, targetable media by its unique keys
	if(!media) {
	...
	*/
	let queue = await createQueue(client,options); // Build and return a valid, reasonable, queue to play
